...
export const VideoPlayer = forwardRef<VideoPlayerHandle, { src: string; poster?: string }>(function VideoPlayer(
  { src, poster },
  ref,
) {
  ...
  return (
    <div className="overflow-hidden rounded-[10px] border border-border-1 bg-black">
      <video ref={videoRef} src={src} controls poster={poster} preload="metadata" className="block max-h-[640px] w-full bg-black" />
    </div>
  );
});
